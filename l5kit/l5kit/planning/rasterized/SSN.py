...
class DWCONV(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        )  # groups, 分组卷积，可以减少参数然后起到加速和正则化效果
        # print(in_channels,out_channels)

    def forward(self, x):
        # print("DWCONVinsize: ",x.shape)
        result = self.depthwise(x)
        # print("DWCONVoutsize:",result.shape)
        return result
...
class LPU(nn.Module):
    ...
    def forward(self, x):
        ...
        # result = self.DWConv(x)
        # print("LPUoutputs: ",result.shape)
        return result
...
class SSNStem(nn.Module):

    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size = 3, stride = 2, padding = 1, bias = False)
        self.gelu1 = nn.GELU() #activate function
        self.bn1 = nn.BatchNorm2d(out_channels) #normalization
